...
- **AI:** Anthropic/Minimax (conversational + action extraction), ElevenLabs (voice transcription)
...
2. Fill in `.env.local` with your keys (Minimax, ElevenLabs, Firebase, Supabase).
...
- `src/app/actions/` — Server actions (e.g. `getAssistantReply`, `transcribeAndExtractActions`)
- `src/lib/anthropic.ts` — Anthropic/Minimax client (server-only)
